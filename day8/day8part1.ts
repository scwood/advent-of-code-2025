...
function main(argv: string[]): void {
  ...
  for (let i = 0; i < 1000; i++) {
    const nextSmallestConnection = connections[i];
  ...
  const remainingCircuits = [...circuitSet];
  remainingCircuits.sort((a, b) => b.boxes.length - a.boxes.length);

  let answer = 1;
  for (let i = 0; i < 3; i++) {
    answer *= remainingCircuits[i].boxes.length;
  }
  console.log(answer);
}
